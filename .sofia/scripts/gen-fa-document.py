...
- Compatible con cualquier proyecto SOFIA (Experis Tracker, BankPortal, etc.)
...
def _add_bookmark(paragraph, bookmark_id):
    p = paragraph._p
    bid = str(abs(hash(bookmark_id)) % 9999999)
...
def toc_line(doc, text, level=1):
    """Linea de TOC manual con tab para numero de pagina."""
    p = doc.add_paragraph()
...
def build_cover(doc, fa, session, doc_version, now_str):
    ...
    for i, (k, v) in enumerate(pairs):
        ...
        set_cell_bg(lc, '1B3A6B')
        set_cell_bg(vc, 'F5F7FA')
        set_cell_borders(lc, '45699E')
        set_cell_borders(vc, 'D9DDE8')
        lc.width = Cm(5)
        vc.width = Cm(9)
        rl = lc.paragraphs[0].add_run(k)
        rl.bold = True; rl.font.color.rgb = WHITE; rl.font.size = Pt(10); rl.font.name = 'Arial'
        rv = vc.paragraphs[0].add_run(v)
        rv.font.color.rgb = BLUE_DARK; rv.font.size = Pt(10); rv.font.name = 'Arial'; rv.bold = True

    doc.add_paragraph()
    p3 = doc.add_paragraph()
    p3.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run3 = p3.add_run(f'Generado por FA-Agent · SOFIA v{SOFIA_VERSION} · {now_str[:16].replace("T"," ")} UTC')
...
def build_toc(doc, fa):
    ...
    # Generar entradas del catalogo por sprint
    ...
    for num, title in sections:
        level = 1 if not num.startswith('  ') else 2
        toc_line(doc, f'{num.strip()}.  {title}', level=level)
...
def build_section1(doc, fa, session):
    ...
    doc_ver   = fa.get('doc_version', '1.0')

    body(doc,
        f'Este documento recoge el Análisis Funcional acumulativo del proyecto {PROJECT_NAME} '
        f'para {CLIENT_NAME}, gestionado bajo la metodología SOFIA v{SOFIA_VERSION} con CMMI Nivel 3 y Scrumban. '
        f'El documento es un artefacto vivo que se actualiza incrementalmente en cada sprint.')
    ...
    # Tabla resumen por sprint desde sprint_history del session
    ...
    for i, (k, h) in enumerate(sorted(sh.items(), key=lambda x: int(x[0].replace('sprint_','').replace('_','.').split('.')[0]) if x[0].replace('sprint_','').split('_')[0].isdigit() else 999)):
...
def build_section2(doc, fa, session):
    ...
    desc = fa.get('description', f'{PROJECT_NAME} es un sistema desarrollado para {CLIENT_NAME} '
                  f'bajo la metodología SOFIA con CMMI Nivel 3. El desarrollo sigue ciclos Scrumban '
                  f'de 2 semanas con pipeline de calidad automatizado (17 steps, 9 gates HITL).')
    body(doc, desc)

    doc.add_paragraph()
    heading2(doc, '2.2 Actores del sistema', bookmark_id='sec2_2')
    actors = fa.get('actors', [
        {'rol': 'Usuario final', 'desc': 'Usuario principal del sistema'},
        {'rol': 'Administrador', 'desc': 'Gestión de configuración y catálogos'},
        {'rol': 'Sistema', 'desc': 'Procesos automáticos y schedulers'},
    ])
    aw = [3.5, 6.5, 4.7]
    ...
    # Leer stack desde sofia-config.json
    ...
    # Regulaciones si las hay en fa-index
...
def build_section3(doc, fa):
    ...
    # Construir mapa de modulos desde fa-index
    ...
    body(doc,
        ...
        '(HLD/LLD) de cada sprint. El flujo general sigue la arquitectura Clean Architecture / '
        'Onion con separación estricta entre Domain, Application e Infrastructure.')
...
def build_section4(doc, fa):
    ...
    # Agrupar funcionalidades por sprint
    ...
    for idx, sp in enumerate(sprints_sorted, 1):
        ...
        h2p = heading2(doc, f'4.{idx}  Sprint {sp} — {feat_label}', bookmark_id=f'sprint_{sp}')

        # Cabecera del sprint
        src_labels = set(f.get('source', f.get('src', 'SPRINT')) for f in funcs)
        src = ', '.join(src_labels)
        src_color = GREEN_SOFT if 'SPRINT' in src or 'EVIDENCIA' in src else (AMBER if 'RECON' in src.upper() else BLUE_MID)
        p = doc.add_paragraph()
        ...
        for f in funcs:
            ...
            r_st.font.size = Pt(9); r_st.font.name = 'Arial'
            r_st.font.color.rgb = GREEN_DARK if 'DELIV' in status.upper() else (AMBER if 'PLAN' in status.upper() else BLUE_MID)
            r_st.bold = True
            ...
            if frns:
                ...
                r_rn.bold = True; r_rn.font.size = Pt(10); r_rn.font.name = 'Arial'; r_rn.font.color.rgb = BLUE_MID
...
def build_section5(doc, fa):
    ...
    total_rn = fa.get('total_business_rules', 0)
    body(doc, f'Catálogo consolidado de {total_rn} reglas de negocio agrupadas por módulo.')
    ...
    for mod, rules in sorted(by_module.items()):
        ...
        for i, rn in enumerate(rules):
            rn_id   = rn.get('id', '—')
            rn_desc = rn.get('desc', rn.get('description', '—'))
            add_data_row(t, [rn_id, rn_desc], rnw, alt=(i%2==1))
        doc.add_paragraph()
...
def build_section6(doc, fa):
    ...
    body(doc, 'Glosario acumulativo de términos de negocio y técnicos del proyecto. Se amplía cada sprint.')

    # Glosario desde fa-index si existe, o genérico
    glossary = fa.get('glossary', [])
    if not glossary:
        glossary = [
            {'term': 'SOFIA', 'def': 'Software Factory IA de Experis — orchestra agentes especializados via MCP'},
            ...
            {'term': 'FA', 'def': 'Functional Analysis — análisis funcional de una característica en lenguaje de negocio'},
            ...
            {'term': 'API REST', 'def': 'Interfaz de programación basada en HTTP con recursos y verbos estándar'},
            {'term': 'JWT', 'def': 'JSON Web Token — token de autenticación sin estado, firmado con clave secreta'},
        ]
...
def build_section7(doc, fa):
    ...
    total_f = fa.get('total_functionalities', 0)
    body(doc, f'Trazabilidad completa de las {total_f} funcionalidades con su sprint, '
              f'feature, estado y tipo de evidencia.')
...
def build_section8(doc, fa, doc_version, now_str):
    ...
    body(doc, 'Registro de versiones del documento de Análisis Funcional. '
              'El documento es un artefacto vivo que se actualiza en cada sprint.')
    ...
    # Historial desde fa-index.json si existe
    history = fa.get('doc_history', [])
    if not history:
        # Construir desde sprint_history implicito
        history = [{'version': '1.0', 'date': fa.get('generated_at', now_str)[:10],
                    'sprint': f"S1–S{fa.get('last_sprint', 1)}", 'desc':
                    f'Creación inicial. FA-Agent activo. {fa.get("total_functionalities",0)} funcionalidades, '
                    f'{fa.get("total_business_rules",0)} reglas de negocio.'}]
    ...
    for i, h in enumerate(history):
        add_data_row(t, [
            ...
            h.get('date', '—')[:10],
            h.get('sprint', '—'),
            h.get('desc', h.get('description', '—')),
        ], hw, alt=(i%2==1))

    # Añadir la version actual como ultima entrada si no esta
    ...
    if last_ver != doc_version:
        add_data_row(t, [
            ...
            f'Actualización automática. {fa.get("total_functionalities",0)} funcionalidades, '
            f'{fa.get("total_business_rules",0)} reglas de negocio. '
    ...
    doc.add_paragraph()
    p = doc.add_paragraph()
    r = p.add_run(f'Documento generado por FA-Agent — SOFIA Software Factory v{SOFIA_VERSION} · {PROJECT_NAME} · {CLIENT_NAME}')
...
def generate(out_path):
    ...
    # Calcular version del documento
    ...
    fa['doc_history'].append({
        ...
        'desc':    f'Actualización automática gate-8b. {fa.get("total_functionalities",0)} func, '
    ...
    if SESSION_PATH.exists():
        try:
            ...
            s['fa_agent']['last_gate']   = '8b'
            s['fa_agent']['last_updated'] = now_str
            s['fa_agent']['doc_version']  = doc_version
            s['fa_agent']['doc_path']     = f'docs/functional-analysis/{DOC_NAME}'
            s['fa_agent']['functionalities'] = fa.get('total_functionalities', 0)
            s['fa_agent']['business_rules']  = fa.get('total_business_rules', 0)
            s['fa_agent']['generator']    = 'python-docx'
            s['fa_agent']['docx_verified'] = True
            s['fa_agent']['docx_size_kb']  = size_kb
            s['fa_agent']['last_sprint_consolidated'] = fa.get('last_sprint', 1)
    ...
    if LOG_PATH.exists():
        try:
            with open(str(LOG_PATH), 'a') as lf:
                lf.write(f'[{now_str}] [FA-AGENT] [GATE-8b] GENERATED → {DOC_NAME} '
                         f'v{doc_version} | {size_kb}KB | {fa.get("total_functionalities",0)} func '
                         f'| {fa.get("total_business_rules",0)} RN | python-docx | SOFIA v{SOFIA_VERSION}\n')
        except Exception: pass
