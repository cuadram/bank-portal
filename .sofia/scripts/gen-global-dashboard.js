...
function buildMAPanel(MA, ORG) {
  ...
  var summary =
    ...
    '<div class="kpi"><div class="kl">Estado global proceso</div>' +
      ...
      '<div class="ks">' + MA.summary.green_count + '🟢 ' + MA.summary.amber_count + '🟡 ' + MA.summary.red_count + '🔴 · Sprint ' + MA.current_sprint + '</div></div>' +
    '<div class="kpi"><div class="kl">Sprints analizados</div>' +
      '<div class="kv kv-blue">' + MA.sprints_analyzed + '</div>' +
      '<div class="ks">' + MA.sprints_range + ' · PA: MA (CMMI L3)</div></div>' +
  ...
  var metricCards = '<div class="g3" style="margin-bottom:18px">' +
    Object.values(MA.metrics).map(function(m) {
      ...
      return '<div class="kpi" style="border-left:3px solid ' + sc + ';padding-left:13px">' +
        ...
        '<div class="ks">mean:' + m.mean + m.unit + ' · p90:' + m.p90 + m.unit + ' · ' + ti + '</div>' +
        '</div>';
    }).join('') + '</div>';
  ...
  var legendOrg = pks.length > 0
    ? '<div style="display:flex;gap:12px;margin-bottom:8px;font-size:10px;color:var(--muted)">' +
      ...
      '</div>'
    : '<div style="font-size:10px;color:var(--dim);margin-bottom:8px">Solo un proyecto · Añade otro para comparar</div>';

  var charts =
    '<div class="g2" style="margin-bottom:16px">' +
    '<div class="card"><div class="ct">Tendencia proceso · ' + MA.sprints_range + '</div>' +
    legendTrend +
    '<div style="position:relative;height:200px"><canvas id="maTrendChart"></canvas></div></div>' +
    '<div class="card"><div class="ct">Comparativa cross-proyecto · ' + pks.length + ' proyecto' + (pks.length !== 1 ? 's' : '') + '</div>' +
    legendOrg +
    '<div style="position:relative;height:200px"><canvas id="maOrgChart"></canvas></div></div>' +
    '</div>';

  var defTable =
    '<div class="card" style="margin-bottom:16px"><div class="ct">Definición métricas MA · umbrales CMMI L3</div>' +
    ...
    '<span>ID</span><span>Nombre</span><span>Fórmula</span><span>Umbral</span><span>Tendencia</span></div>' +
  ...
  if (ORG && ORG.org_baseline && pks.length > 0) {
    ...
    orgTable =
      '<div class="card"><div class="ct">ORG Baseline Experis · ' + pks.length + ' proyecto' + (pks.length !== 1 ? 's' : '') + ' registrado' + (pks.length !== 1 ? 's' : '') + '</div>' +
      ...
      '<span>Métrica</span><span>Nombre</span>' + pks.map(function(pk){return '<span>' + pk + '</span>';}).join('') + '<span>ORG MEAN</span></div>' +
      Object.entries(ORG.org_baseline).map(function(pair) {
        ...
        return '<div style="display:grid;grid-template-columns:' + cols + ';gap:8px;padding:8px 0;border-bottom:1px solid var(--border);font-size:11px;align-items:center">' +
          ...
          pks.map(function(pk) {
            ...
            return '<span style="font-family:var(--mono);color:' + psc + '">' + pd.mean + u + '<br><span style="font-size:9px;color:var(--dim)">' + pd.current_status + ' · ' + (pd.trend || '') + '</span></span>';
          }).join('') +
...
function T(id){
  ...
  const ids=['exec','history','quality','roadmap','gobierno','gates','ma'];
