...
// Ejecutar: node gen_all_word.js
// Prerequisito: npm install docx (en directorio sprint-2-FEAT-001 o globalmente)
'use strict';
...
const SPRINTS = {
  4: {
    ...
    periodo:'2026-05-05 → 2026-05-16', fecha:'2026-05-16', sp:24,
    dir:'sprint-4-FEAT-003',
    ...
    items:[
      ['US-201','Gestionar dispositivos de confianza (ver/eliminar)','4','new-feature','✅ DONE'],
      ...
      ['DEBT-004','ua-parser-java para fingerprint preciso (WARN-F2-002)','2','tech-debt','✅ DONE'],
    ...
    gates:[
      ...
      ['Arquitectura','LLD-003-trusted-devices.md','Tech Lead','✅ APPROVED'],
      ['Code Review','CR-FEAT-003-sprint4.md','Tech Lead','✅ APPROVED'],
      ['QA Gate','QA-REPORT-sprint4.md','QA Lead','✅ APPROVED'],
      ['Go/No-Go STG','RELEASE-v1.3.0.md','Release Manager','✅ APPROVED'],
    ],
    risks:[
      ['R-S4-001','Fingerprint cambia al actualizar browser','M','B','🟢 Baja','Re-enrollment automático informativo','✅ CERRADO'],
      ['R-S4-002','Job nocturno falla — dispositivos no expirados','B','B','🟢 Baja','Alert + manual fallback','✅ CERRADO'],
      ['R-003','TOTP_TEST_SECRET pendiente CI','M','A','🟠 Alta','Pendiente DevOps','⚠️ ABIERTO'],
    ],
    pci:[['8.6.1','Dispositivos de acceso gestionados','✅ CUMPLE'],['10.2.7','Cambios de config. de autenticación','✅ CUMPLE']],
    endpoints:[
      ['GET /api/v1/trusted-devices','Listar dispositivos del usuario'],
      ['POST /api/v1/trusted-devices','Agregar dispositivo (OTP required)'],
      ['DELETE /api/v1/trusted-devices/{id}','Revocar dispositivo'],
      ['DELETE /api/v1/trusted-devices/all','Revocar todos'],
    ],
  },
  5: {
    feat:'FEAT-005', titulo:'Panel de Auditoría (inicio)', version:'v1.4.0',
    periodo:'2026-05-26 → 2026-06-06', fecha:'2026-06-06', sp:24,
    dir:'sprint-5-FEAT-005',
    goal:'"Implementar el Dashboard de Seguridad con resumen visual y la exportación de historial de eventos (PDF/CSV), cubriendo PCI-DSS 4.0 req. 10.7 — acceso del titular al historial de actividad."',
    items:[
      ['US-401','Dashboard de seguridad con resumen visual actividad','4','new-feature','✅ DONE'],
      ['US-402','Exportar historial eventos en PDF y CSV','3','new-feature','✅ DONE'],
      ['DEBT-005','SecurityDashboard 6 queries CompletableFuture.allOf()','3','tech-debt','✅ DONE'],
      ...
      ['ACT-HLD','HLD FEAT-005 + diagrama C4 Context/Containers','2','documental','✅ DONE'],
      ['ACT-LLD','LLD-005 backend + frontend US-401/402','2','documental','✅ DONE'],
      ['ACT-ADR','ADR-009 dual HMAC key rotation','1','documental','✅ DONE'],
      ['ACT-OpenAPI','OpenAPI v1.4.0 endpoints FEAT-005','1','documental','✅ DONE'],
      ['ACT-E2E','Playwright E2E dashboard + exportación','3','testing','✅ DONE'],
    ],
    gates:[
      ['Sprint Planning','SPRINT-005-planning.md','Product Owner','✅ APPROVED'],
      ['HLD + ADR','HLD-FEAT-005 + ADR-009','Tech Lead','✅ APPROVED'],
      ['LLD + LLD Review','LLD-005-backend + LLD-005-frontend','Tech Lead','✅ APPROVED'],
      ['Code Review','CR-FEAT-005-sprint5.md','Tech Lead','✅ APPROVED'],
      ['QA Gate','QA-REPORT-sprint5.md','QA Lead','✅ APPROVED'],
    ],
    risks:[
      ['R-F5-001','PDF síncrono bloquea thread en exports grandes','M','B','🟢 Baja','@Async + streaming timeout 30s','✅ CERRADO'],
      ['R-F5-002','Hash SHA-256 en PDF — usuario no sabe verificar','B','B','🟢 Baja','Instrucciones en el PDF','✅ CERRADO'],
      ['R-F5-003','Prefs notif desactivan eventos relevantes — PCI gap','M','M','🟡 Media','Separar visible/audit_log','✅ CERRADO'],
      ['R-003','TOTP_TEST_SECRET pendiente CI','M','A','🟠 Alta','Pendiente DevOps','⚠️ ABIERTO'],
    ],
    ...
    endpoints:[
      ['GET /api/v1/security/dashboard','Dashboard resumen actividad'],
      ['GET /api/v1/security/export','Exportar historial PDF/CSV'],
      ['GET /api/v1/security/config-history','Historial cambios configuración'],
    ],
  },
  6: {
    feat:'FEAT-005+004', titulo:'Preferencias + Notificaciones inicio', version:'v1.5.0',
    periodo:'2026-06-09 → 2026-06-20', fecha:'2026-06-20', sp:24,
    dir:'sprint-6-FEAT-005',
    ...
    items:[
      ...
      ['US-301','Historial paginado notificaciones 90 días','4','new-feature','✅ DONE'],
      ['US-302','Mark-as-read individual + bulk','2','new-feature','✅ DONE'],
      ['US-303','Badge unread-count @Cacheable 30s','3','new-feature','✅ DONE'],
      ['DEBT-008','SecurityDashboard CompletableFuture.allOf() optimización','3','tech-debt','✅ DONE'],
      ['Flyway-V7','Flyway V7 user_notifications base','2','infra','✅ DONE'],
      ['ACT-ADR-010','ADR-010 SSE CDN/proxy + ADR-011 JWT context-pending','2','documental','✅ DONE'],
      ['ACT-LLD-006','LLD-006 NotificationCenter backend + frontend','2','documental','✅ DONE'],
      ['ACT-OpenAPI-141','OpenAPI v1.4.1 FEAT-005 cierre','1','documental','✅ DONE'],
      ['ACT-E2E','E2E preferencias + notificaciones inicio','2','testing','✅ DONE'],
    ],
    gates:[
      ['Sprint Planning','SPRINT-006-planning.md','Product Owner','✅ APPROVED'],
      ['ADR-010/011','ADR-010 + ADR-011 context-pending JWT','Tech Lead','✅ APPROVED'],
      ['LLD-006','LLD-006 NotificationCenter','Tech Lead','✅ APPROVED'],
      ['Code Review','CR-Sprint6.md','Tech Lead','✅ APPROVED'],
      ['QA Gate','QA-REPORT-sprint6.md','QA Lead','✅ APPROVED'],
    ],
    risks:[
      ['R-S6-001','SSE connections — pool sin límite inicial','M','M','🟡 Media','ADR-012 define pool Sprint 8','✅ CERRADO'],
      ['R-S6-002','JWT context-pending reutilizable','M','A','🟠 Alta','DEBT-009 Sprint 8 cierra','⚠️ ABIERTO'],
    ],
    pci:[['8.6.3','Preferencias autenticación gestionadas','✅ CUMPLE'],['10.2.1','Cambios config registrados','✅ CUMPLE']],
    endpoints:[
      ['GET /api/v1/security/preferences','Ver preferencias de seguridad'],
      ['PUT /api/v1/security/preferences','Actualizar preferencias'],
      ['GET /api/v1/notifications','Historial notificaciones paginado'],
      ['GET /api/v1/notifications/unread-count','Badge count'],
    ],
  },
  7: {
    feat:'FEAT-006', titulo:'Autenticación Contextual y Bloqueo de Cuenta', version:'v1.7.0',
    periodo:'2026-06-09 → 2026-06-23', fecha:'2026-06-23', sp:24,
    dir:'sprint-7-FEAT-006',
    ...
    items:[
      ['US-601','Bloqueo automático tras 10 intentos fallidos (ventana 24h)','5','new-feature','✅ DONE'],
      ['US-602','Desbloqueo por enlace email TTL 1h (anti-enumeration 204)','3','new-feature','✅ DONE'],
      ...
      ['ACT-30','OpenAPI v1.4.0 claims JWT + ADR-011 + LLD-006/007','1','documental','✅ DONE'],
    ],
    gates:[
      ['Sprint Planning','SPRINT-007-planning.md','Product Owner','✅ APPROVED'],
      ['ADR-011 + LLD-006/007','ADR-011 context-pending JWT + LLD-006/007','Tech Lead','✅ APPROVED'],
      ['Code Review S1','US-601 + DEBT-008 + US-403','Tech Lead','✅ APPROVED'],
      ['Code Review S2','US-602 + US-603 + US-604','Tech Lead','✅ APPROVED'],
      ['QA Gate','QA-REPORT-sprint7-semana2.md — 43 tests, 0 defectos','QA Lead','✅ APPROVED'],
    ],
    risks:[
      ['R-S7-001','JWT context-pending reutilizable sin blacklist','M','A','🟠 Alta','DEBT-009 Sprint 8 prioritario','⚠️ ABIERTO'],
      ['R-S7-002','extractIpSubnet duplicado en varios servicios','B','B','🟢 Baja','DEBT-010 Sprint 8','⚠️ ABIERTO'],
    ],
    ...
    endpoints:[
      ['POST /api/v1/account/unlock','Solicitar enlace desbloqueo (204 anti-enum)'],
      ['GET /api/v1/account/unlock/{token}','Desbloquear cuenta desde email'],
      ['POST /api/v1/auth/confirm-context','Confirmar subnet nueva (scope=context-pending)'],
      ['GET /api/v1/security/config-history','Historial cambios configuración 90d'],
    ],
  },
  8: {
    feat:'FEAT-004', titulo:'Centro de Notificaciones de Seguridad', version:'v1.8.0',
    periodo:'2026-03-03 → 2026-03-17', fecha:'2026-03-17', sp:24,
    dir:'sprint-8-FEAT-004',
    ...
    items:[
      ...
      ['US-303','Badge unread-count @Cacheable 30s + estado inicial SSE','3','new-feature','✅ DONE'],
      ['US-304','Acciones directas: 12 deep-links + revoke session directo','4','new-feature','✅ DONE'],
      ['US-305','SSE 1 conn/usuario + heartbeat 30s + fallback polling 60s','3','new-feature','✅ DONE'],
      ['DEBT-009','JWT blacklist Redis TTL=JWT-restante + SseRegistry.invalidate()','3','tech-debt','✅ DONE'],
      ['DEBT-010','extractIpSubnet /24 centralizado en DeviceFingerprintService','2','tech-debt','✅ DONE'],
      ['Flyway-V9','V9 user_notifications + notification_preferences + 3 índices','2','infra','✅ DONE'],
      ['ACT-31','OpenAPI v1.5.0 FEAT-004 + SSE + JWT blacklist documentado','1','documental','✅ DONE'],
    ],
    gates:[
      ['Sprint Planning','SPRINT-008-planning.md','Product Owner','✅ APPROVED'],
      ['ADR-012 + LLD-008/009','SseRegistry pool + JWT blacklist + SSE','Tech Lead','✅ APPROVED'],
      ['Code Review S1','DEBT-009/010 + Flyway V9 + US-301/302/303','Tech Lead','✅ APPROVED'],
      ['Code Review S2','US-304 + US-305 + OpenAPI v1.5.0','Tech Lead','✅ APPROVED'],
      ['QA Gate','QA-REPORT-sprint8.md — 59 tests, 0 defectos','QA Lead','✅ APPROVED'],
      ['DevOps','Commit 53f0305 + rama ready for merge v1.8.0','DevOps','✅ APPROVED'],
    ],
    risks:[
      ['R-S8-001','SSE no escala horizontalmente sin Redis Pub/Sub','M','M','🟡 Media','DEBT-011 Sprint 9','⚠️ ABIERTO'],
      ['R-S8-002','Purga user_notifications falta job nocturno','B','B','🟢 Baja','DEBT-012 Sprint 9','⚠️ ABIERTO'],
    ],
    pci:[['8.3','JWT blacklist tras uso único','✅ CUMPLE'],['10.2.1','Auditoría por consulta de notificaciones','✅ CUMPLE']],
    endpoints:[
      ['GET /api/v1/notifications','Historial paginado con filtros'],
      ['PUT /api/v1/notifications/{id}/read','Marcar individual como leída'],
      ['PUT /api/v1/notifications/read-all','Marcar todas como leídas'],
      ['GET /api/v1/notifications/unread-count','Badge count (cacheable 30s)'],
      ['POST /api/v1/notifications/{id}/revoke-session','Revocar sesión desde notificación'],
      ['GET /api/v1/notifications/stream','SSE stream tiempo real'],
    ],
  },
};
...
const mkRiskTable = risks => new Table({ width:{size:9026,type:WidthType.DXA}, columnWidths:[900,2600,400,400,900,1926,900], rows:[
  new TableRow({children:['ID','Riesgo','P','I','Exp.','Mitigación','Estado'].map((t,i)=>HC(t,[900,2600,400,400,900,1926,900][i]))}),
  ...risks.map(([id,r,p,imp,exp,plan,st],i)=>new TableRow({children:[DC(id,900,i%2===0?C.VL:C.WHITE,true),DC(r,2600,i%2===0?C.VL:C.WHITE),DC(p,400,i%2===0?C.VL:C.WHITE),DC(imp,400,i%2===0?C.VL:C.WHITE),DC(exp,900,SF(exp)),DC(plan,1926,i%2===0?C.VL:C.WHITE),DC(st,900,SF(st))]})),
]});
...
async function genSprint(snum) {
  ...
  const saveTo = async (doc, fname) => { const buf = await Packer.toBuffer(doc); fs.writeFileSync(path.join(WORD_DIR, fname), buf); console.log('  ✅', path.join(WORD_DIR, fname)); };
  ...
  const srDoc = mkDoc(`BankPortal — Sprint Report Sprint ${snum}`, s.fecha, [
    ...
    H('1. Metadata', 2),
    MT([['Proyecto','BankPortal — Banco Meridian'],['Sprint',`Sprint ${snum}`],['Feature',`${s.feat} — ${s.titulo}`],['Período',s.periodo],['Estado','✅ CERRADO — Sprint Goal ALCANZADO'],['Versión',s.version]]), SP(),
    H('2. Sprint Goal', 2),
    new Paragraph({ shading:{fill:C.VL,type:ShadingType.CLEAR}, border:{left:{style:BorderStyle.SINGLE,size:12,color:C.BLUE}}, spacing:{before:100,after:100}, indent:{left:200,right:200}, children:[new TextRun({text:s.goal,font:'Arial',size:20,italics:true,color:C.BLUE})] }),
    ...
    new Table({ width:{size:9026,type:WidthType.DXA}, columnWidths:[3200,2000,2000,1826], rows:[
      ...
      ...[['Story Points','24','24','0 SP'],['US completadas',`${s.items.filter(([,,,t])=>t==='new-feature').length}`,`${s.items.filter(([,,,t])=>t==='new-feature').length}`,'0'],['Defectos QA','0','0','—'],].map(([m,p,r,v],i)=>new TableRow({children:[DC(m,3200,i%2===0?C.VL:C.WHITE,true),DC(p,2000,i%2===0?C.VL:C.WHITE),DC(r,2000,i%2===0?C.VL:C.WHITE),DC(v,1826,i%2===0?C.VL:C.WHITE)]})),
    ]}), SP(),
    ...
    new Table({ width:{size:9026,type:WidthType.DXA}, columnWidths:[1400,5826,1800], rows:[
      new TableRow({children:['Requisito','Descripción','Estado'].map((t,i)=>HC(t,[1400,5826,1800][i]))}),
      ...s.pci.map(([r,d,st],i)=>new TableRow({children:[DC(r,1400,i%2===0?C.VL:C.WHITE,true),DC(d,5826,i%2===0?C.VL:C.WHITE),DC(st,1800,SF(st))]})),
    ]}), SP(),
  ...
  const rrDoc = mkDoc(`BankPortal — Risk Register Sprint ${snum}`, s.fecha, [
    ...
    MT([['Total riesgos',String(s.risks.length)],['Abiertos',String(s.risks.filter(([,,,,,, st])=>st.includes('ABIERTO')).length)],['Cerrados',String(s.risks.filter(([,,,,,, st])=>st.includes('CERRADO')).length)]]), SP(),
    P(`SOFIA SM Agent — Experis · ${s.fecha}`,{color:'666666',italics:true}),
  ]);
  ...
  const rnDoc = mkDoc(`BankPortal — Release Notes ${s.version}`, s.fecha, [
    ...
    H('1. Metadata', 2),
    MT([['Versión',s.version],['Fecha',s.fecha],['Sprint',`Sprint ${snum}`],['Feature',`${s.feat} — ${s.titulo}`]]), SP(),
    ...
    new Table({ width:{size:9026,type:WidthType.DXA}, columnWidths:[1000,6526,1500], rows:[
      ...
      ...s.items.filter(([,,,t])=>t==='new-feature').map(([id,ti,sp],i)=>new TableRow({children:[DC(id,1000,i%2===0?C.VL:C.WHITE,true),DC(ti,6526,i%2===0?C.VL:C.WHITE),DC(sp,1500,i%2===0?C.VL:C.WHITE)]})),
    ]}), SP(),
    H('3. Nuevos Endpoints', 2),
    new Table({ width:{size:9026,type:WidthType.DXA}, columnWidths:[3000,6026], rows:[
      new TableRow({children:['Endpoint','Descripción'].map((t,i)=>HC(t,[3000,6026][i]))}),
      ...s.endpoints.map(([ep,d],i)=>new TableRow({children:[DC(ep,3000,i%2===0?C.VL:C.WHITE,true),DC(d,6026,i%2===0?C.VL:C.WHITE)]})),
    ]}), SP(),
    H('4. Deuda Técnica Resuelta', 2),
    new Table({ width:{size:9026,type:WidthType.DXA}, columnWidths:[1200,7826], rows:[
      new TableRow({children:['ID','Descripción'].map((t,i)=>HC(t,[1200,7826][i]))}),
      ...s.items.filter(([,,,t])=>t==='tech-debt').map(([id,ti],i)=>new TableRow({children:[DC(id,1200,i%2===0?C.VL:C.WHITE,true),DC(ti,7826,i%2===0?C.VL:C.WHITE)]})),
    ]}), SP(),
